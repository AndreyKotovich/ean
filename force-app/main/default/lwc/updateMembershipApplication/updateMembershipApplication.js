...
export default class UpdateMembershipApplication extends NavigationMixin(LightningElement) {
    ...
    handleCountryPrices() {
        ...
        getDeadline()
            .then(result => {
                ...
                //Determine the most expensive contact's membership
                let activeMembershipApiName;
                let maxPrice;
                if(this.contactMemberships.length>0){
                    for(let membership of this.contactMemberships){
                        let foundMembershipPrice = false;
                        let defaultMembershipPrice = 0;

                        for(let countryPrice of this.countryPrices){

                            if (membership.Membership__c === countryPrice.Membership__c) {

                                if(countryPrice.Region__c === 'Default'){
                                    defaultMembershipPrice = countryPrice[result];
                                }

                                if (countryPrice.Region__c === this.region) {
                                    foundMembershipPrice = true;
                                    let price = countryPrice[result];
                                    if (maxPrice == null || maxPrice < price) {
                                        maxPrice = price;
                                        activeMembershipApiName =  membership['Membership__r']['API__c'];
                                    }
                                }
                            }
                        }

                        if(!foundMembershipPrice){
                            if (maxPrice == null || maxPrice < defaultMembershipPrice) {
                                maxPrice = defaultMembershipPrice;
                                activeMembershipApiName =  membership['Membership__r']['API__c'];
                            }
                        }
                    }
                }
                //determined the most expensive membership
                for(let membership of this.allAvailableMemberships){
                    let isFound = false;
                    for(let countryPrice of this.countryPrices){
                        if (membership.Id === countryPrice.Membership__c && membership.isForUpdateMembershipApplication__c) {
                            if (countryPrice.Region__c === this.region) {
                                isFound = true;
                                radioButtonsLabelsPromises.push(this.getRadioButtonLabel(countryPrice[result], membership, activeMembershipApiName, result));
                            }
                        }
                    }
    ...
    getRadioButtonLabel(initialPrice, membership, activeMembershipApiName, deadline){
        return new Promise((resolve, reject) => {
            let radioButtonLabel = {
                membershipId: membership.Id,
                membershipName: membership.Name,
                membershipAPI: membership.API__c,
                activeMembership: activeMembershipApiName,
    ...
    generateOrderWithItems(){
        return new Promise((resolve, reject) => {
            if(!this.isEdit){
                generateOrder({ContactId: this.currentContactId, orderItemsObject: this.generateOrderItems(), purchaseType:'Update'})
                    .then(orderId => {
    ...
    manageMarkup(){
        ...
        if(!!!activeMembership && this.contactMemberships.length===1){
            activeMembership = this.contactMemberships[0]['Membership__r']['API__c'];
        }
        ...
        switch (activeMembership) {
            ...
            case 'full_membership':
                ...
                break;
            case 'resident_and_research_membership':
                this.residentAndResearchMembershipExist();
                break;
            case 'student_membership':
                this.studentMembershipExist();
                break;
        }
    ...
    disableEnableMemberships(currentMembership, membershipToEnable){
        return new Promise((resolve, reject) => {
            try{
                for(let item of this.contactMemberships){
                    let membershipId = item['Membership__c'];
                    if (membershipId == currentMembership.Id) {
                        for(let item2 of membershipToEnable){
                            this.availableMembershipsForApplying[item2.Id] = true;
                            let htmlMembershipElement = this.template.querySelector('input[title="' + item2.Id + '"]');
                            if (htmlMembershipElement !== null && htmlMembershipElement.hasAttribute('disabled')) {
